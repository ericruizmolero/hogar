...
export const parseIdealistaHtml: PlatformParser = (html: string, providedUrl: string): Partial<Property> => {
  // Crear un DOM parser
  ...
  const hasFullRenovation = fullText.includes('reforma integral') || fullText.includes('reforma total') || fullText.includes('reformar completamente');
  const hasAnyRenovation = fullText.includes('a reformar') || fullText.includes('para reformar') || fullText.includes('necesita reforma');
  const needsRenovation = hasFullRenovation ? 'total' as const : hasAnyRenovation ? 'partial' as const : 'no' as const;
